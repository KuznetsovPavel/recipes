...
class SpoonacularLoaderTest extends AsyncFunSuite with Matchers {
  ...
  test("load by id") {
    ...
    val backend = SttpBackendStub
      .asynchronousFuture
      .whenRequestMatches(_.uri.toJavaUri == new URI("https://api.spoonacular.com/recipes/10/information?apiKey=someApi"))
      .thenRespond(data)
  ...
  test("backend return error") {
    ...
    val loader = new SpoonacularLoader[Future](backend, "someApi")
    recoverToSucceededIf[SpoonacularError](loader.getRecipe(10))
  }
